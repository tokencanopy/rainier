...
- `type agentEntry struct{ CredentialProtocol uint64; Provider, Dir string; Files []string }` decoded from `RAINIER_AGENTS_B64`; unsupported or absent credential protocols are dropped so mixed versions cannot bypass a logout fence.
...
`0011_agent_credential_revokes.sql` adds `revoked` and `last_revoked_version`; a revoke clears the sealed bytes, advances both the row version and the durable fence, and later puts preserve the fence. Migration 0011 requires a stop-the-world `controld` rollout because a pre-0011 binary deletes logout rows. The matching session image must be published and existing sessions replaced before login/logout traffic resumes.
