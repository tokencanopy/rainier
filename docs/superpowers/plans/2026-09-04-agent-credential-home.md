...
**A5 (found by the probes, not planned):** with the session's egress proxy configured through `HTTPS_PROXY`, Claude Code 2.1.197 (the npm build on Node 20) completes login, self-update checks, and the OAuth exchange through the proxy, but its **model API calls do not go through it**: every attempt fails with `Connection error` and `API Error: Unable to connect to API (ConnectionRefused)` after eleven retries, with one proxied CONNECT to `api.anthropic.com` in the audit log where dozens should be, and the session network has no DNS (`getent hosts api.anthropic.com` answers nothing, which is expected on the internal bridge). Codex could not be tested past login because `chatgpt.com` was not on the probe's allowlist. The claude.ai MCP connectors also bypass the proxy and cost about 58 s of retries per start unless `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` is set. **This blocks the dogfood, not the plan:** the mount, the sync, and custody are unaffected, but Task 6's real login must be preceded by a probe that makes a model call succeed — candidates, in order: a newer Claude Code (the native build, or a release that routes the API client through the proxy agent), `NODE_USE_ENV_PROXY=1` on a Node that supports it, or a DNS answer plus a transparent egress path on the session bridge. The `claude` provider row's `Egress` and `Env` are finalized only when that probe passes.
