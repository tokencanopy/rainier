...
| D2 | `POST /v0/sessions` accepts `image`, `cmd`, and `egress_allow` together with `environment` (the flags compose, `cmd/rainier/main.go:575`) | `400 invalid_request` "an environment session cannot override the image, command, or egress allowlist" — refused loudly, never dropped silently | `control.CreateSession.Validate` refuses environment + any scratch field. **Awaiting Josh's decision:** the CLI's designed composition contradicts the frozen contract; the likely resolution is a coordinated `control` change letting an environment session carry overrides, made before this branch merges | `api_test.go` cases that send an override with an environment |
...
| D11 | `POST /v0/sessions` with neither `environment` nor `image` (nor `cmd`/`egress_allow`) is accepted; the runner boots its default image (`rainier new` with no flags sends exactly this) | `400 invalid_request` "a session must name an environment or an image" | `controlapp.CreateSession` refuses a create with neither an environment nor a scratch spec — a rule the Wave 4 review added. **Awaiting Josh's decision** together with D2: restore the runner-default-image create in `controlapp`, or keep the refusal and change the CLI | `api_test.go` `TestCreateSession` cases that post only a name (given an image in the interim) |
