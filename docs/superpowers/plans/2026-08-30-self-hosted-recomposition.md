...
|---|---|---|---|---|
| D1 | Runner/sandbox free text reaches HTTP error bodies (`writeSandboxErr`, `runner reported failure: <detail>` in logs); a secret that vanishes between create and dispatch fails the session with the secret's name in its `Error` column | Handlers map the seven sentinels to fixed status/slug/message; a session's own `Error` column (written by `ApplyRunnerEvent`) still carries the runner's failure tail, but a launch-material failure records `controlapp`'s fixed sentence — the create-time preflight (Task 4) still names a missing secret in its 409 | A provider-neutral service does not relay runner text (control/errors.go) | `api_test.go`, `files_test.go`, `srpc_test.go` assertions on error *message* text only, plus one assertion each in `internal/cli/client_test.go` and `internal/e2e/e2e_test.go` that expected the sandbox's own sentence |
| D2 | `POST /v0/sessions` accepts `image`, `cmd`, and `egress_allow` together with `environment` (the flags compose, `cmd/rainier/main.go:575`) | **Resolved by rainier #31**: the contract now composes an environment with per-session overrides, so the branch's interim 400 is removed and the flows work as before — with one refinement, per-session `egress_allow` extends the environment's list instead of replacing it | The frozen rule was a design guess the product contradicted; the first host composed against it showed that | `api_test.go` cases restored to today's assertions, egress case to the extend semantics |
| D3 (restored by plan 8, `2026-08-30-control-outbox-checkpoint-capabilities.md` Task 4 — deviation D17) | A session from an environment with a current snapshot boots from the snapshot only if the snapshot runner is connected and has room, else from the plain image anywhere | Such a session is placed only on the snapshot runner (capability `snapshot:<runner>`); if that runner is full it stays queued | `control.Environment` has no snapshot-runner field; the checkpoint plan (O9) restores portable placement | `api_test.go`/`sched_test.go` cases asserting the busy-runner fallback |
...
| D11 | `POST /v0/sessions` with neither `environment` nor `image` (nor `cmd`/`egress_allow`) is accepted; the runner boots its default image (`rainier new` with no flags sends exactly this) | **Resolved by rainier #31**: `controlapp` no longer refuses a scratch create with no image — it asks the host for its default, as today | Same decision as D2: the host's policy, not the portable contract, decides whether an image is required | `api_test.go` `TestCreateSession` cases restored to posting only a name |
| D13 | (controlapp, not the wire) the 900s setup/init timeout defaults and "a bound travels only with a hook that runs" were `createSpec` policy in controld's scheduler; `controlapp` copied the environment's values verbatim, so a create with no declared bound carried `0` and an init bound was sent without an init hook | `FleetOptions.DefaultSetupTimeoutSec`/`DefaultInitTimeoutSec` (additive host policy, composed as 900/900); `createSpec` applies a bound only to a hook it dispatches | The wire and the runner-side bound are unchanged from today | none |
| D14 | (controlapp, not the wire) an announce that agrees with the store made a same-state transition to bump `last_event_at`; `controlapp` skipped it, so a session nothing else reports on stalled | The same-state transition is made | `last_event_at` is the "demonstrably alive" signal the session view shows | `controlapp/fleet_test.go` idempotency cases counting transitions |
| D15 | (controlapp, not the wire) a live row the store had placed **nowhere** (requeued while its runner was away) was **destroyed** when that runner announced it; controld adopted it onto the announcing runner in the announced state | Adopted, as today | The extraction's duplicate branch treated an empty runner as a mismatch; that tears down live work | none |
| D12 | (controlapp seam, not the wire) a runner that answers a lifecycle command with `OK: false` is `500 internal`; one that never answers is `502 runner_unreachable` | Unchanged on the wire. `controlapp` exports `ErrRunnerRefused`, which wraps `control.ErrUnavailable` (so `errors.Is(err, control.ErrUnavailable)` still holds) and is what `dispatch` returns for a false result; the handler maps it to 500 before `unavailableStatus` | Retry-versus-investigate is a signal today's wire carries and the closed sentinel set cannot express alone | none |
...
- Modify: `controlapp/sessions.go` (D8, D10, D12) and `controlapp/sessions_test.go` (`TestLifecycleRunnerUnavailable` → the new behavior, the full-pool cases → queued, plus three new tests)
...
**D12** in `controlapp/sessions.go`: add `var ErrRunnerRefused = fmt.Errorf("controlapp: runner refused the command: %w", control.ErrUnavailable)` and return it from `dispatch` when `res.OK` is false (transport errors and a missing connection keep returning bare `ErrUnavailable`). Test that a false result satisfies both `errors.Is(err, control.ErrUnavailable)` and `errors.Is(err, ErrRunnerRefused)`, and that a transport error satisfies only the former. In `writeSessionErr`, check `ErrRunnerRefused` first → `500 internal` with the handler's existing "could not … session" message, then `unavailableStatus` for the rest.

**D3, create side:** `controlapp.portableSpecFor` resolves an environment session to its current snapshot regardless of the snapshot runner's connectivity or room, so the two `…ResolvesEnvironment/… falls back to image and setup` cases change here; rewrite them to assert the resolution only and drop their placement assertions, which Task 5 restores once the `snapshot:` capability pins placement.
...
- Modify: `internal/controld/runners.go` — `handleRunnerConnect`, `readLoop`, `touchRunner`, `connectRunner`, `retireRunner` kept and reshaped; **delete** `applyEvent`, `placedExactlyOn`, `reconcile`, `reconcilePresent`, `reconcileMissing`, `reconcileUnplaced`, `transitionQuiet`, `announcedState`, `dispatch`, `drain`; **keep** `destroyOrphan` (over `s.transport.Dispatch`), `failStage`/`stageFailure` (detail composition), `cacheEnvironment`, `snapshotWanted`, `buildSnapshot`, `snapshotRef`, `rejectCredential`, `runnerConn` and its loops, `sendToRunner`, `broadcastToRunners`, `conn`, `isCurrentConn`, `nameLock`, `registerRunner`, `runnerTokenOK`, `closeRunner`, `clip`
...
for _, id := range res.Destroy { s.destroyOrphan(connCtx, name, string(id)) } // destroyOrphan is kept: destroy to the runner (it has never sent remove_workspace, which belongs to the delete path), no store write, now over s.transport.Dispatch
s.fleet.Wake(installPool)
s.readLoop(connCtx, rc)
```

`announcedSessions` maps `runner.SessionInfo{ID, State}` to `control.RunnerSession{SessionID, State: control.SessionState(State)}`. `envID`, whose only caller is the create preflight in `api.go`, moves out of `sched.go` when `sched.go` is deleted (Task 7 may settle it in `adapt_launch.go`).
...
Settled during execution: the handler keeps a pre-upgrade owner-or-admin check (`mayAttach`) that asks the same `ownerOrAdmin` adapter, so a refused attach is a 403 before any socket exists as today; `readFirstResize` is deleted because the broker reads the first message off the port; a sandbox's `ok:false` surfaces from `controlapp.sessionRPC` as `ErrRunnerRefused` (D12) and the three workspace handlers keep it a `409 conflict` with a fixed sentence; the open-transfer bound is 64, refused with `409 conflict` rather than a 429 the wire has never carried; the transfer limit travels through `Config.MaxTransferBytes`, replacing `Server.xferMax`; an idle transfer is closed by a per-transfer timer on the pairing TTL, not a sweeper.
