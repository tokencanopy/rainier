...
```bash
./bin/rainier ls --all                # the failed session (--verbose adds its runner)
...
| 8 | Riders land: a container crash preserves the workspace volume (salvage until `rm`); `child_exited` is visible in `ls` (today: in the Process column, separately from lifecycle and reachability); `attach --since 0` full-history replay actually reaches the viewer; `/v0/me` exposes the user id (and the CLI's owner-preference uses it). | `go test ./internal/e2e/ -run 'TestCrashKeepsTheWorkspaceAndRmReclaimsIt|TestEnvSetupStreamsAndCaches|TestFullReplayReachesTheViewer'`. On the VM, the `--since 0` half is the one Plan 3's overnight run found broken — re-run it there. | ☑ | `attach --since 0` replayed the full boot output on rainier-1; the other riders passed their automated scenes. The pre-existing `gce-1` session also survived the full-stack Plan 5 redeploy without a container restart. |
...
| 2 | An established attach reconnects after a transient transport/control-plane interruption; detach and process exit do not reconnect. | `go test -race ./internal/attachio ./cmd/rainier -run 'TestRunNoRaceOnFloodedOutputDuringDetach|TestRunReportsDisconnectCursor|TestAttachWithRetryReconnectsFromTheRenderedCursor|TestAttachWithRetryBacksOffTransientDisconnects|TestAttachWithRetryDoesNotRetryPolicyClose|TestDiscardPendingInputFlushesTTYButPreservesPipes|TestRunDiscardsTTYInputQueuedDuringDial'`. Repeat with a controld restart during a live manual attach when an isolated local fleet is available. | ◐ | Automated real-WebSocket boundaries are green under the race detector: reconnect resumes at the acknowledged cursor, repeated transient closes back off, policy closes stop, and input typed during both reconnect delay and a paused handshake is discarded immediately before input ownership while scripted pipes remain intact. Manual restart rehearsal remains pending because this machine's shared local fleet already owns the fixed rehearsal ports. |
| 3 | Reconnect uses the last frame actually rendered, including a snapshot; a drop before the first frame preserves the caller's original cursor. | `go test ./internal/attachio -run 'TestRunReportsDisconnectCursor|TestRunPreservesSinceAllBeforeFirstFrame|TestRunDoesNotAdvanceCursorAfterShortOutputWrite'`. | ☑ | Snapshot seq 17 becomes the cursor; ordinary and full-replay pre-frame cursors survive; a short local output write does not acknowledge its frame; an exit reports its code and never enters retry. |
