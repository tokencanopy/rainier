...
func (s *Server) createSpec(ctx context.Context, row Session, env *Environment) (*rwire.Spec, string) {
	...
	if fail := s.applyRepos(ctx, row, env, spec); fail != "" {
		return nil, fail
	}
	if env == nil {
		return spec, ""
	...
	// init is the other half of that split, and it has no cache condition at
	// all: it runs after the clones, against code no image can contain, so
	// EVERY create dispatches it — the cache-booted ones above all, since
	// those are the sessions whose setup did not run this time round (design
	// §4.4). It is deliberately absent from SetupHash for the same reason:
	// editing it cannot invalidate an image it never touched.
	if env.Init != "" {
		spec.Init = env.Init
		spec.InitTimeoutSec = env.InitTimeoutSec
		if spec.InitTimeoutSec <= 0 {
			spec.InitTimeoutSec = defaultInitTimeoutSec
		}
	}
...
// ---------------------------------------------------------------------------
// repo resolution (design §4.3)
//
// One list of repositories, two places that need it: the create, which gates
// on a credential and records the egress a clone needs, and the dispatch,
// which expands it into the clone instructions a sandbox executes. Both ask
// sessionRepoRefs, so the question "does this session clone" cannot be
// answered one way at create and another at dispatch.
// ---------------------------------------------------------------------------
...
// applyRepos fills in the clone half of row's Spec: the resolved repositories,
// the hosts they need, and the identity their commits carry. It returns a
// client-facing failure reason, empty when there is nothing to fail.
//
// The egress append happens here as well as at create because the two can
// legitimately disagree: an environment that gained a github connector while
// this session sat in the queue is dispatched with repositories the row's
...
func (s *Server) applyRepos(ctx context.Context, row Session, env *Environment, spec *rwire.Spec) string {
	...
	if err != nil {
		...
		return "could not resolve the repositories this session clones"
	}
	if len(refs) == 0 {
		return ""
	}

	spec.Repos = expandRepos(row, refs)
	spec.EgressAllow = withGitHubHosts(spec.EgressAllow)

	// Attribution is resolved from the owner's user row at dispatch, not
	// copied onto the session at create: it is derived data, and a login the
	// user has since changed on GitHub is re-read here rather than frozen into
	// every session they ever started.
	u, err := s.st.GetUser(ctx, row.OwnerID)
	if err != nil {
		log.Printf("controld: create %s: reading owner %s for git attribution: %v", row.ID, row.OwnerID, err)
		return "could not resolve the git identity this session commits as"
	}
	spec.GitAuthorName = u.Login
	spec.GitAuthorEmail = noreplyEmail(u)
	return ""
}
