...
//
// The identity and vault cases are the ones RunContract has always run, word
// for word: the same behavior seen through the narrower interface, still
// answering with the store's own sentinels while the old Store methods are
// alive. The four lookup cases are new and answer with the control sentinel
// set, as every native method does from the start.
//
