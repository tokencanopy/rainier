...
- **Terminal Ownership:** [`docs/terminal-controller-ownership.md`](docs/terminal-controller-ownership.md) — who may type when several devices are attached to one session, and how the three parties that implement it interoperate across releases. By default input is **shared**: every attached terminal may type, no claims and no take-over key. `controld --input-policy exclusive` selects one-controller-at-a-time instead ([design](docs/design/2026-09-12-shared-attachment-policy.md))
...
carry what a host needs beyond the services: [`runnerplane`](runnerplane/doc.go)
(the runner WebSocket endpoint, generation fencing, dispatch correlation),
[`attachplane`](attachplane/doc.go) (the terminal attach handshake and
splice), and [`v0wire`](v0wire/doc.go) (the `/v0/` JSON shapes and the
