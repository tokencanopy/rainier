...
async function fixture(t) {
  ...
  const release = { version: '0.0.8', platform: 'linux', arch: 'x64', url: 'https://example.test/release.tar.gz', archiveHash: hash(bytes), binaryHash: hash(binary) };
  return { dir, binary, bytes, release, cache: join(dir, 'cache'), download: async () => bytes };
}

test('maps all supported platforms to immutable v0.0.8 assets', () => {
  for (const platform of ['darwin', 'linux']) {
    for (const [arch, goArch] of [['arm64', 'arm64'], ['x64', 'amd64']]) {
      const release = selectRelease(platform, arch);
      assert.equal(release.url, `https://github.com/tokencanopy/rainier/releases/download/v0.0.8/rainier_0.0.8_${platform}_${goArch}.tar.gz`);
