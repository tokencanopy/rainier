...
Or pin this release: `npx @tokencanopy/rainier@0.0.3 --help`.
...
The first invocation downloads the matching native v0.0.3 binary from GitHub
...
[standalone release binary](https://github.com/tokencanopy/rainier/releases/tag/v0.0.3).
...
Version 0.0.3 is beta: interfaces may change. npm's `beta` tag selects the beta
...
or version. There is no native CLI auto-updater or version subcommand in v0.0.3.

## Maintainers

From the repository root, run `make verify` and `make npm-test`. Smoke-test an
...
Never replace v0.0.3 assets or mutate their pinned hashes after publication;
